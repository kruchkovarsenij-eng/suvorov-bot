...
def format_report(session):
    ...
    name = answers[0] if answers[0] else "Аноним"
    lines = []
    lines.append(f"📋 ДИАГНОСТИКА — «Пластик Руси»")
    lines.append(f"👤 {name}")
    lines.append(f"🕐 {datetime.now().strftime('%d.%m.%Y %H:%M')}")
    lines.append(f"📱 @{session.get('username', '—')} | ID: {session['user_id']}")
    lines.append("")
...
async def send_report(bot, session):
    report = format_report(session)
    chunks = [report[i:i+4000] for i in range(0, len(report), 4000)]
    for j, chunk in enumerate(chunks):
        prefix = "🏁 ОТЧЁТ ДИАГНОСТИКИ\n\n" if j == 0 else ""
        await bot.send_message(chat_id=ADMIN_CHAT_ID, text=prefix + chunk)


async def ask_question(update_or_query, context, session, edit=False):
    idx = session["current"]
    q = QUESTIONS[idx]
    total = len(QUESTIONS)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await ask_question(update, context, session)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session["current"] >= len(QUESTIONS):
        await finish(update, context, session)
    else:
        await ask_question(update, context, session)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session["current"] >= len(QUESTIONS):
        await finish(update, context, session, from_callback=True)
    else:
        await ask_question(query, context, session)


async def finish(update, context, session, from_callback=False):
    ...
    await send_report(context.bot, session)
